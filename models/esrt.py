from models import esrt_common as common
import torch
import torch.nn as nn
import torch.nn.functional as F
from thop import profile
from esrt_util.non  import NONLocalBlock2D
from esrt_util.tools import extract_image_patches,\
    reduce_mean, reduce_sum, same_padding, reverse_patches

from esrt_util.transformer import drop_path, DropPath, PatchEmbed, Mlp,MLABlock
from esrt_util.position import PositionEmbeddingLearned, PositionEmbeddingSine
import pdb
import math
...
class Un(nn.Module):
    def __init__(self,n_feats, wn):
        ...
        # self.reduce = common.default_conv(n_feats, n_feats, 3)
        self.weight2 = common.Scale(1)
        self.weight1 = common.Scale(1)
        self.attention = MLABlock(n_feat=n_feats, dim= 144) 
    ...
    def forward(self,x):
        ...
        x_calc = self.reduce(torch.cat([x1,x2,x3],dim=1))
        # print("before passing to multi-head attention")
        # print(x_calc.shape)

        out = self.attention(self.reduce(torch.cat([x1,x2,x3],dim=1)))


        # print("after comming out of attention block")
        # print(out.shape)

        out = out.permute(0,2,1)
        out = reverse_patches(out, (h,w), (3,3), 1, 1)

        # print("reverse patch shape", out.shape)
...
class ESRT(nn.Module):
    def __init__(self, upscale=4, n_feats= 16, n_blocks=1, kernel_size=3, conv=common.default_conv):
        ...
        self.scale = upscale  #args.scale[0] #gaile
        act = nn.ReLU(True)
        ...
        # define head module
        modules_head = [conv(1, self.n_feats, self.kernel_size)]
        
        # define body module
        modules_body = nn.ModuleList()
        for i in range(n_blocks):
            modules_body.append(
                Un(n_feats=self.n_feats, wn = wn))

        # define tail module
        modules_tail = [
            common.Upsampler(conv, self.scale, self.n_feats, act=False),
            conv(self.n_feats, 1,self.kernel_size)]


        self.up = nn.Sequential(common.Upsampler(conv,self.scale,self.n_feats,act=False),
                          BasicConv(self.n_feats, 1,3,1,1))
        self.head = nn.Sequential(*modules_head)
        self.body = nn.Sequential(*modules_body)
        self.tail = nn.Sequential(*modules_tail)
        self.reduce = conv(self.n_blocks*self.n_feats, self.n_feats, self.kernel_size)

        # print("The head structure")
        # print(self.head)
    ...
    def forward(self, x1,x2 = None, test=False):
        # x1 = self.sub_mean(x1)
        x1 = self.head(x1)
        # print("output of head :", x1.shape)
        res2 = x1
        #res2 = x2
        body_out = []
        for i in range(self.n_blocks):
            x1 = self.body[i](x1)
            # print("shape of body out", x1.shape)
            body_out.append(x1)
        res1 = torch.cat(body_out,1)
        res1 = self.reduce(res1)
        # print("shape after reduce res1", res1.shape)

        
        x1 = self.tail(res1)
        # print("shape of tail", x1.shape)
        x1 = self.up(res2) + x1
        # x1 = self.add_mean(x1)
        #x2 = self.tail(res2)
        return x1
