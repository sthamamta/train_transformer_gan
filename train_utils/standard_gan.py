...
from train_utils.general import log_results, LogOutputs, create_loss_meters_from_keys, LogMetric, hfen_error
import os
import torch
...
class StandardGANTrainer(Trainer):
    ...
    #             d_loss_sr = self.adversarial_criterion(fake_output,real_labels) 
    ...
    #             loss_D_real = self.adversarial_criterion(gt_output, real_labels) * 0.5
    #             (loss_D_real).backward(retain_graph=True)

    #             sr_output = self.D(fake_images.detach().clone())
    #             loss_D_fake = self.adversarial_criterion(sr_output, fake_labels) 
